...
---
Based on feedback from John Field, it was suggested that we consider including some length based information to describe mesopealgic prey (and perhaps contrast to CPS)
NOTE: For more detail on downloading data from ERDDAP, see MesoAsPrey_ERDDAP.Rmd

#update
Started by II 1/18/23, updated 2/1/23 following feedback from John and Jarrod that the plots looked ok.
- going to update these plots from distribution curves to regular histograms
...
Whereas the original version I donwloaded has length data available for predators, there isn't length data available for prey. That requires re-downloading data directly from ERDDAP in a different format. 

The only coding difference is that we write distinct=FALSE instead of true
...
and had to change length input fields 
...
# Could always play around with different fields here, but I think I at least have all of the length columns available
```
...
however, there are in some cases alternative measuremnts withint the database
prey_measurement_2
prey_length_2


Next, would like to see what size info is available for meso fish...
...
lengths_meso_cps |> 
  ...
  summarise(n=n()) |>  # total number of records of prey measurements per spp and measurement type
  filter(prey_measurement_1=="TL")


```
So there isn't a lot of consistency in length measurements. 
...
#  Try to find ANY length info - combine different measurements 
...
# Now filter for those values that are missing SL measurement-- still 23,292
...
This should be able to work with case_when() but the function wouldn't recognize my calculations in the second term, so sticking with clunky but works ifelse statements :) 
```{r}
...
WOOHOO now have a fully updated df with the column length (SL in mm) and length_type (either measured, backcalcualted or estimated from a different length measurement)

# still missing 21,794 lengths for prey items
but have a total of 27,460 records of length for both cps fish and mesopealgic fish prey!

```{r}

# I went thru and did a series of checks ot ensure that there wasn't an avaialble length in prey_measurement_2
# only a total of 13 values where there wasn't a SL estimate from measurement, back calculation or estimate from other length but was info on FL, TL or SL in prey_measurement_2
...
# from 13 down to 6 - i feel like I have done everythign I can here to provide as much data as possible!!
...
# ESTIMATE the mean length per family of mesopelagic fish 
...
Double checked and the predators that we had length data for were all, with the exception of hte blue blue shark Prionace glauca recorded as having eaten mesopealgic fishes
- not quite sure why so few meso fish were measured 3267 compared to 27467 in database, but so it goes!
...
ggplot(data=lengths_meso_morethan3, aes(x=length, group=family))+
  geom_histogram(aes(fill=family), color="black", alpha=0.25, position="nudge", bins=25)+
  scale_fill_viridis_d(option="D")+ #set color scale
  labs(fill="Mesopelagic fish family", x="Length (mm)", y="Number of fish", size=2 )+
  scale_x_continuous( expand= c(0,10), breaks=seq(0, 500, by=25))+
